...
GLOBAL_REINFORCEMENT_INC=$(read_config '.global_instincts.reinforcement_increment // 0.15' "$PROJECT_ID" 2>/dev/null || echo "0.15")
GLOBAL_REINFORCEMENT_INC=$(validate_numeric "$GLOBAL_REINFORCEMENT_INC" "$_NUMERIC_NONNEG_FLOAT" "0.15")
GLOBAL_MAX_CONFIDENCE=$(read_config '.global_instincts.max_confidence // 0.95' "$PROJECT_ID" 2>/dev/null || echo "0.95")
GLOBAL_MAX_CONFIDENCE=$(validate_numeric "$GLOBAL_MAX_CONFIDENCE" "$_NUMERIC_NONNEG_FLOAT" "0.95")
